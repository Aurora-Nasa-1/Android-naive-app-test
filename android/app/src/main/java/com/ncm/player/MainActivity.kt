...
fun AppNavigation(
    ...
    val hasBottomBar = showNav // Show bottom bar (Nav or Docked) if logged
    ...
    Scaffold(
        ...
        bottomBar = {
            if (showNav && !useSideNav && !isPlayerExpanded && !isLyricsExpanded) {
    ...
    SharedTransitionLayout(modifier = Modifier.fillMaxSize()) {
        Box(
            ...
            Row(modifier = Modifier.fillMaxSize()) {
                if (loginViewModel.isLogged && !isPlayerExpanded && !isLyricsExpanded && useSideNav) {
                    ...
                    if (expanded) {
                        ...
                        Box(
                            modifier = Modifier
                                ...
                                .offset {
                                    if (useSideNav) IntOffset.Zero else IntOffset(0, -bottomBarOffsetHeightPx.value.toInt())
                                }
                        ) {
                            BottomPlaybackBar(
                                sharedTransitionScope = this@SharedTransitionLayout,
